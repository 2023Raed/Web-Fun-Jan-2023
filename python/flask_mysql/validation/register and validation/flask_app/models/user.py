...
class User():
    ...
    def create_user(cls,data):
        query="INSERT INTO signers (first_name,last_name,email,password) VALUES (%(first_name)s,%(last_name)s,%(email)s,%(password)s);"
    ...
    def get_signers(cls):
        ...
        signers=[]
        for row in results:
            signers.append(cls(row))
        return signers
    
    #get one user by id
    @classmethod
    def get_by_id(cls,data):
        query="SELECT * FROM signers WHERE id=%(id)s;"
    ...
    def get_by_email(cls,data):
        query=" SELECT * FROM signers WHERE email=%(email)s;"
        result= connectToMySQL(DATABASE).query_db(query,data)
